...
const VERSION = "v3-stripe-zarinpal-2026-09-03";
...
async function handlePayment(request, env) {
  ...
  if (currency === "IRR") {
    if (!env.ZARINPAL_MERCHANT_ID) {
      return err("درگاه پرداخت ریالی هنوز پیکربندی نشده است. ZARINPAL_MERCHANT_ID را تنظیم کنید.", 500);
    }
    ...
    try {
      const zpRes = await fetch("https://api.zarinpal.com/pg/v4/payment/request.json", {
        ...
        body: JSON.stringify({
          ...
          amount: amount * 10, // Zarinpal expects Rial, plan is stored in Toman
          description: plan.title,
          callback_url: `${origin}/api/payment/callback/zarinpal?pid=${paymentId}`,
      ...
      console.error("Zarinpal error:", JSON.stringify(zpData));
      return err("خطا در ایجاد لینک پرداخت زرین‌پال.", 502);
    } catch (e) {
      console.error(e);
      return err("خطا در ارتباط با درگاه پرداخت ریالی.", 502);
    }
  }

  // ---- USD: Stripe Checkout ----
  if (!env.STRIPE_SECRET_KEY) {
    return err("درگاه پرداخت دلاری هنوز پیکربندی نشده است. STRIPE_SECRET_KEY را تنظیم کنید.", 500);
  }

  try {
    const params = new URLSearchParams();
    params.set("mode", "payment");
    params.set("success_url", `${origin}/?payment=success&pid=${paymentId}`);
    params.set("cancel_url", `${origin}/?payment=cancelled&pid=${paymentId}`);
    params.set("client_reference_id", paymentId);
    params.set("customer_email", user.email);
    params.set("line_items[0][quantity]", "1");
    params.set("line_items[0][price_data][currency]", "usd");
    params.set("line_items[0][price_data][unit_amount]", String(Math.round(amount * 100))); // cents
    params.set("line_items[0][price_data][product_data][name]", plan.title);
    ...
    if (!stripeRes.ok) {
      ...
      return err(stripeData?.error?.message || "خطا در ایجاد لینک پرداخت دلاری.", 502);
    }

    await env.DB
      .prepare("UPDATE payments SET authority = ? WHERE id = ?")
      .bind(stripeData.id, paymentId)
      .run();

    return json({ ok: true, payment_url: stripeData.url });
  } catch (e) {
    console.error(e);
    return err("خطا در ارتباط با درگاه پرداخت دلاری.", 502);
  }
}
...
async function handleZarinpalCallback(request, env) {
  ...
  if (env.ZARINPAL_MERCHANT_ID) {
    ...
    if (verifyData?.data?.code === 100 || verifyData?.data?.code === 101) {
      await creditUserForPayment(env, payment, pid, String(verifyData.data.ref_id || ""));
...
// Stripe sends server-to-server webhooks — this is the reliable way to confirm payment,
// rather than trusting the success_url redirect alone (which a user could fake).
async function handleStripeWebhook(request, env) {
  if (!env.STRIPE_WEBHOOK_SECRET) {
    return err("Stripe webhook پیکربندی نشده است.", 500);
  }
  ...
  if (!isValid) return err("امضای وبهوک نامعتبر است.", 400);
  ...
  if (event.type === "checkout.session.completed") {
    ...
    if (paymentId) {
      ...
      if (payment && payment.status !== "success") {
        await creditUserForPayment(env, payment, paymentId, session.payment_intent || "");
      }
    }
  }
...
async function creditUserForPayment(env, payment, paymentId, refId) {
  await env.DB
    .prepare("UPDATE payments SET status = 'success', ref_id = ? WHERE id = ?")
    .bind(refId, paymentId)
    .run();

  const plan = PLANS.find((p) => p.id === payment.plan_id);
  const creditAmount = plan ? plan.irr : payment.amount; // balance is tracked in Toman

  await env.DB
    .prepare("UPDATE users SET balance = balance + ? WHERE id = ?")
    .bind(creditAmount, payment.user_id)
    .run();
  ...
  await env.DB
    ...
    .bind(uuid(), payment.user_id, creditAmount)
    .run();
}
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (method === "GET" && path === "/api/payment/callback/zarinpal") return await handleZarinpalCallback(request, env);
      if (method === "POST" && path === "/api/payment/webhook/stripe") return await handleStripeWebhook(request, env);
...
<p class="small" style="text-align:center;margin-top:20px">نسخه: ${VERSION}</p>
</div>

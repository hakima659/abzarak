...
<nav>
<button type="button" onclick="showPage('home')">🏠 حساب</button>
<button type="button" onclick="showPage('ai')">🤖 هوش مصنوعی</button>
<button type="button" onclick="showPage('plans')">💰 پلن‌ها</button>
<button type="button" onclick="showPage('payment')">💳 پرداخت</button>
<button type="button" onclick="showPage('withdraw')">💸 برداشت</button>
<button type="button" onclick="showPage('adminLogin')">🛠️ مدیریت</button>
...
<button type="button" class="primary" onclick="login()">ورود</button>

<div id="loginMsg"></div>

<p class="center">
<button type="button" class="secondary" onclick="showPage('register')">
ثبت‌نام
</button>

<button type="button" class="secondary" onclick="showPage('forgot')">
بازیابی رمز
</button>
</p>
</section>
...
<button type="button" class="primary" onclick="register()">
ثبت‌نام
</button>
...
<p class="center">
<button type="button" class="secondary" onclick="showPage('login')">
بازگشت به ورود
</button>
</p>
</section>
...
<button type="button" class="primary" onclick="forgot()">
دریافت کد بازیابی
</button>
...
<button type="button" class="primary" onclick="resetPassword()">
تغییر رمز
</button>
...
<p class="small">
سؤال خودت را بنویس؛ پاسخ توسط Workers AI تولید می‌شود.
</p>
...
<textarea
  id="question"
  placeholder="مثلاً: برای کسب درآمد اینترنتی چه راه‌هایی پیشنهاد می‌کنی؟"
></textarea>

<button type="button" class="primary" onclick="askAI()">
ارسال سؤال 🤖
</button>
...
<button type="button" class="primary" onclick="createPayment()">
ایجاد سفارش پرداخت
</button>
...
<input
  id="withdrawAddress"
  placeholder="آدرس USDT یا اطلاعات حساب بانکی"
>

<button type="button" class="primary" onclick="withdraw()">
ثبت درخواست برداشت
</button>
...
<button type="button" class="primary" onclick="adminLogin()">
ورود مدیریت
</button>
...
<button type="button" onclick="adminWithdrawals()">
💸 برداشت‌ها
</button>

<button type="button" onclick="adminPayments()">
💳 پرداخت‌ها
</button>
...
window.register=async function(){
  ...
  try{
    ...
    msg.innerHTML=
      '<div class="msg success">ثبت‌نام با موفقیت انجام شد.</div>';
...
window.login=async function(){
  ...
  try{
    ...
    msg.innerHTML=
      '<div class="msg success">ورود موفق بود.</div>';
...
window.loadUser=async function(){
  ...
  try{
    ...
    $("profile").innerHTML=
      "<b>"+
      escapeHTML(d.user.name || d.user.email)+
      "</b><br>"+
      "📧 "+
      escapeHTML(d.user.email)+
      "<br>"+
      "📌 وضعیت: "+
      escapeHTML(d.user.status || "فعال")+
      "<br>"+
      "📦 پلن: "+
      escapeHTML(d.user.plan || "free");

    $("balance").textContent=
      Number(d.user.balance || 0)
...
window.loadTransactions=async function(){
  ...
  try{
    ...
    var d=await api("/api/transactions");
...
window.askAI=async function(){
  ...
  if(!q){
    ...
    $("aiMsg").innerHTML=
      '<div class="msg error">سؤال را وارد کن.</div>';
  ...
  $("aiMsg").innerHTML=
    '<div class="msg">🤖 در حال فکر کردن...</div>';

  try{

    var d=await api("/api/ai",{
      method:"POST",
      body:JSON.stringify({
        question:q
      })
...
window.loadPlans=async function(){
  ...
  try{
    ...
    $("plansBox").innerHTML=
      html ||
      '<div class="msg">پلنی موجود نیست.</div>';
...
window.loadPlansSelect=async function(){
...
window.createPayment=async function(){
  ...
  try{
    ...
    $("paymentMsg").innerHTML=
      ...
      '✅ سفارش ایجاد شد<br>'+
      'شماره سفارش: '+
      escapeHTML(d.payment_id)+
      '<br>'+
      'پلن: '+
      escapeHTML(d.plan.title)+
      '<br>'+
      'مبلغ: '+
...
window.withdraw=async function(){
  ...
  try{
    ...
    $("withdrawMsg").innerHTML=
      '<div class="msg success">'+
      '✅ درخواست برداشت ثبت شد.<br>'+
      'شماره درخواست: '+
      escapeHTML(d.withdrawal_id)+
      '</div>';
...
window.resetPassword=async function(){
  ...
  try{
    ...
    var d=await api("/api/reset-password",{
      method:"POST",
      body:JSON.stringify({
...
window.adminLogin=async function(){
  ...
  try{
    ...
    var d=await api("/api/admin/login",{
      method:"POST",
      body:JSON.stringify({
...
window.adminWithdrawals=async function(){
  ...
  try{
    ...
    var d=await api("/api/admin/withdrawals");
...
window.adminPayments=async function(){
  ...
  try{
    ...
    var d=await api("/api/admin/payments");
...
async function handle(request,env){
  ...
  if(
    ...
    if(!env.AI){
      ...
      return json({
        ok:false,
        error:
          "اتصال Workers AI با نام AI پیدا نشد"
      },500);
          ...
          messages:[
            {
              role:"system",
              content:
                "تو یک دستیار هوش مصنوعی فارسی‌زبان مفید، دقیق و محترمانه هستی. پاسخ‌ها را واضح، کاربردی و به زبان فارسی بده."
            },
            {
              role:"user",
              content:question
  ...
  if(
    ...
    const plan=
      PLANS.find(function(x){
        return String(x.id)===planId;
      });
  ...
  if(
    ...
    if(!address){
      ...
      return json({
        ok:false,
        error:
          "آدرس یا اطلاعات حساب را وارد کنید"
      },400);

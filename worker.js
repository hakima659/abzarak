...
.plan{border:2px solid #e5e7eb;border-radius:16px;padding:18px}
.plan strong{font-size:22px;display:block;margin:8px 0}
input,select,button{width:100%;padding:13px;margin:7px 0;border-radius:10px;border:1px solid #d7dce5;font-size:15px}
...
.hidden{display:none}
.msg{padding:12px;border-radius:10px;margin:8px 0;background:#eef2ff}
...
<button onclick="showPanel('payments')">💳 پرداخت</button>
...
<div class="card">
<h2>💰 موجودی</h2>
<div id="balance" class="balance">در حال بارگذاری...</div>
</div>
...
<p>🇮🇷 ایران: تومان | 🌎 خارج از ایران: دلار</p>
...
async function register(){
 try{
  ...
  msg("regMsg","✅ ثبت‌نام با موفقیت انجام شد.");
  $("registerBox").classList.add("hidden");
...
async function loadUser(){
 try{
  ...
  $("profile").innerHTML="❌ خطا: "+e.message;
...
async function createPayment(){
 ...
 try{
  ...
  msg(
   "paymentMsg",
   "✅ سفارش ثبت شد.<br>"+
   "شماره سفارش: "+data.payment_id+
   "<br>مبلغ: "+data.amount+" "+data.currency+
   "<br><small>درگاه واقعی هنوز متصل نشده است.</small>"
  );
...
async function adminUsers(){
 ...
 try{
  ...
  $("adminContent").innerHTML=
   '<div class="card">❌ خطا در بارگذاری کاربران:<br>'+
   e.message+'</div>';
...
async function adminWithdrawals(){
 ...
 try{
  ...
  $("adminContent").innerHTML=
   '<div class="card">❌ خطا:<br>'+e.message+'</div>';
...
async function handle(request,env){
 ...
 if(path==="/api/withdraw" && request.method==="POST"){
  ...
  if(!address){
   return json({
    ok:false,
    error:"آدرس برداشت را وارد کنید"
   },400);
  }

...
//   DB -> D1 database binding (already exists: f2d229a0-4b87-4273-89b8-aa2532fdae64)
//   AI -> Workers AI binding (free tier, no external API key needed)
//         Add via: Workers & Pages > your worker > Settings > Bindings > Add > Workers AI
// =============================================================
...
async function verifyPassword(password, stored) {
  ...
  const saltBytes = new Uint8Array(parts[2].match(/.{1,2}/g).map((b) => parseInt(b, 16)));
  const keyMaterial = await crypto.subtle.importKey(
    "raw", new TextEncoder().encode(password), { name: "PBKDF2" }, false, ["deriveBits"]
  );
  const derivedBits = await crypto.subtle.deriveBits(
  ...
  const hashHex = [...new Uint8Array(derivedBits)].map((b) => b.toString(16).padStart(2, "0")).join("");
  if (hashHex.length !== parts[3].length) return false;
  let diff = 0;
  for (let i = 0; i < hashHex.length; i++) diff |= hashHex.charCodeAt(i) ^ parts[3].charCodeAt(i);
  return diff === 0;
}
...
async function getUserFromToken(request, env) {
  ...
  const user = await env.DB.prepare("SELECT id, name, email, balance FROM users WHERE id = ?").bind(session.user_id).first();
  return user || null;
}
...
async function handleSignup(request, env) {
  ...
  if (!isValidEmail(email)) return json({ error: "فرمت ایمیل نامعتبر است" }, 400);
  ...
  if (existing) return json({ error: "این ایمیل قبلاً ثبت‌نام کرده است" }, 409);
  ...
  ).bind(userId, name || "", normalizedEmail, passwordHash, 0, createdAt).run();

  const token = uuid() + uuid();
  await env.DB.prepare("INSERT INTO sessions (id, user_id, token, created_at) VALUES (?, ?, ?, ?)")
    .bind(uuid(), userId, token, createdAt).run();

  return json({ success: true, user: { id: userId, name: name || "", email: normalizedEmail, balance: 0 }, token });
}
...
async function handleLogin(request, env) {
  ...
  if (!isValid) {
    if (!user.password_hash.startsWith("pbkdf2:")) {
      return json({ error: "رمز عبور این حساب نیاز به بازیابی دارد (فرمت قدیمی)" }, 401);
    }
  ...
  const token = uuid() + uuid();
  await env.DB.prepare("INSERT INTO sessions (id, user_id, token, created_at) VALUES (?, ?, ?, ?)")
    .bind(uuid(), user.id, token, new Date().toISOString()).run();

  return json({ success: true, user: { id: user.id, name: user.name, email: user.email, balance: user.balance }, token });
}

async function handleMe(request, env) {
  const user = await getUserFromToken(request, env);
  if (!user) return json({ error: "نشست نامعتبر یا منقضی شده است" }, 401);
  return json({ user });
}

// ---------------- AI chat route (Cloudflare Workers AI — free tier) ----------------

async function handleAiChat(request, env) {
  const user = await getUserFromToken(request, env);
  if (!user) return json({ error: "برای استفاده از هوش مصنوعی ابتدا وارد حساب شوید" }, 401);

  let body;
  try { body = await request.json(); } catch { return json({ error: "بدنه درخواست نامعتبر است" }, 400); }
  const { message } = body;
  if (!message || typeof message !== "string") return json({ error: "پیام الزامی است" }, 400);

  if (!env.AI) {
    return json({ error: "سرویس هوش مصنوعی هنوز تنظیم نشده — باید Binding با نام AI به این Worker اضافه شود" }, 503);
  }

  try {
    const aiResponse = await env.AI.run("@cf/meta/llama-3.1-8b-instruct", {
      messages: [
        { role: "system", content: "تو یک دستیار هوشمند فارسی‌زبان هستی. مفید، دقیق و مودب پاسخ بده." },
        { role: "user", content: message },
      ],
    });

    return json({ reply: aiResponse.response || "پاسخی دریافت نشد." });
  } catch (err) {
    return json({ error: "خطا در ارتباط با سرویس هوش مصنوعی: " + err.message }, 500);
  }
}
...
<style>
  * { box-sizing: border-box; }
  ...
  .bubble { padding: 10px 14px; border-radius: 12px; max-width: 85%; }
...
<main>
  ...
  <div id="view-ai" class="card hidden">
    ...
    <input id="ai-input" type="text" placeholder="پیام خود را بنویسید...">
    <div class="actions">
...
function showView(name) {
  const views = ['login', 'signup', 'account', 'ai', 'plans'];
  views.forEach(v => document.getElementById('view-' + v).classList.add('hidden'));
  if ((name === 'account' || name === 'ai') && !token) name = 'login';
...
async function loadAccount() {
  ...
  document.getElementById('account-info').innerHTML =
    '<p><b>نام:</b> ' + (data.user.name || '-') + '</p>' +
    '<p><b>ایمیل:</b> ' + data.user.email + '</p>' +
    '<p><b>موجودی:</b> ' + data.user.balance + '</p>';
}
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/signup" && request.method === "POST") return handleSignup(request, env);
    if (url.pathname === "/api/login" && request.method === "POST") return handleLogin(request, env);
    if (url.pathname === "/api/me" && request.method === "GET") return handleMe(request, env);
    if (url.pathname === "/api/plans" && request.method === "GET") return handlePlans(request, env);
